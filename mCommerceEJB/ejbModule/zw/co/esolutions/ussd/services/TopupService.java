...
import zw.co.esolutions.ussd.util.AuthUtil;
...
public class TopupService extends USSDService {
		...
		try {
			...
			if(session.getLevel() == 1){
				...
				if(SystemConstants.EXIT.equals(option)){
					...
					String authResp = AuthUtil.validateMobile(response);
...
//					session.addUserInput(SystemConstants.TARGET_MOBILE, response);
//					session = gotoLevelThree(session, null);
					return session;
				}else{
	...
	public static USSDSession gotoLevelOne(USSDSession session,String menu){
		...
		session.setServiceName(SystemConstants.SERVICE_NAME_TOPUP_ANOTHER_PHONE);
	...
	public static USSDSession gotoLevelTwo(USSDSession session,String menu){
		...
		if(menu == null){
			menu = config.getStringValueOf(SystemConstants.ENTER_TARGET_MOBILE_MSG);
		} 
		menu += config.getStringValueOf(SystemConstants.BACK_EXIT_MSG);
		session.addBackAndExitToOptionsMenu();
		session.setLevel(2);
