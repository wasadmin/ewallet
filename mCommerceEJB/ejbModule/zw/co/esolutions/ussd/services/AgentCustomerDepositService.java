...
public class AgentCustomerDepositService extends USSDService {
		...
		try {
			...
			if(session.getLevel() == 1){
				...
				if(SystemConstants.EXIT.equals(option)){
					...
					session = gotoLevelOne(session, config.getStringValueOf(SystemConstants.SELECT_ACCOUNT_DEPOSIT_INVALID_OPTION_MSG));
					return session;
				}
				...
				if(SystemConstants.EXIT.equals(option)){
					...
					String authResp = AgentAuth.validateAgentDeposit(session.getMobileNumber(), response);
					
					if(ResponseCode.E000.name().equalsIgnoreCase(authResp)) {
						try {
							session.addUserInput(SystemConstants.TARGET_MOBILE, NumberUtil.formatMobileNumber(response));
						} catch (Exception e) {
					...
					session = gotoLevelTwo(session, config.getStringValueOf(SystemConstants.ENTER_TARGET_MOBILE_INVALID_OPTION_MSG));
					return session;
				}
	...
	public static USSDSession gotoLevelOne(USSDSession session,String menu){
		...
		if(menu == null){
			menu = config.getStringValueOf(SystemConstants.SELECT_ACCOUNT_DEPOSIT_MSG);
		}
		...
		session.setServiceName(SystemConstants.SERVICE_NAME_DEPOSIT_CASH);
	...
	public static USSDSession gotoLevelTwo(USSDSession session,String menu){
		...
		if(menu == null){
			menu = config.getStringValueOf(SystemConstants.ENTER_DEPOSIT_TARGET_MOBILE_MSG);
		} 
		menu += config.getStringValueOf(SystemConstants.BACK_EXIT_MSG);
		session.addBackAndExitToOptionsMenu();
		session.setLevel(2);
	...
	public static USSDSession gotoLevelThree(USSDSession session,String menu){
		...
		if(menu == null){
			menu = config.getStringValueOf(SystemConstants.DEPOSIT_ENTER_AMOUNT_MSG);
		} 
		menu += config.getStringValueOf(SystemConstants.BACK_EXIT_MSG);
		session.addBackAndExitToOptionsMenu();
		session.setLevel(3);
	...
	public static USSDSession gotoLevelFour(USSDSession session,String menu){
		...
		if(menu == null){
			menu = config.getStringValueOf(SystemConstants.CONFIRM_DEPOSIT);
		} 
		menu += "\n Deposit of " + NumberUtils.getFormattedAmount(session.getInput(SystemConstants.AMOUNT)) + 
		" by " + session.getInput(SystemConstants.TARGET_MOBILE) +  "\n Agent : " + session.getInput(SystemConstants.AGENT_NUMBER);
	...
	public static USSDSession processTransaction(USSDSession session){
		...
		msg.setTargetMobileNumber(session.getInput(SystemConstants.TARGET_MOBILE));
