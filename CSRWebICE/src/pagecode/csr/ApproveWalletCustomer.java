/**
 * 
 */
package pagecode.csr;

import javax.faces.component.html.HtmlCommandButton;
import javax.faces.component.html.HtmlForm;
import javax.faces.component.html.HtmlMessages;
import javax.faces.component.html.HtmlOutputLabel;
import javax.faces.component.html.HtmlOutputText;
import javax.faces.component.html.HtmlPanelGrid;
import javax.faces.component.html.HtmlPanelGroup;

import pagecode.PageCodeBase;

import com.ibm.faces.component.html.HtmlScriptCollector;
import com.icesoft.faces.component.ext.HtmlCommandLink;
import com.icesoft.faces.component.ext.HtmlDataTable;
import com.icesoft.faces.component.ext.UIColumn;
import com.icesoft.faces.component.paneltabset.PanelTab;

/**
 * @author stanford
 *
 */
public class ApproveWalletCustomer extends PageCodeBase {

	protected PanelTab bfpanel4;
	protected HtmlForm form1;
	protected HtmlScriptCollector scriptCollector1;
	protected HtmlMessages messages1;
	protected PanelTab bfpanel1;
	protected HtmlPanelGrid grid1;
	protected HtmlOutputLabel label2;
	protected HtmlOutputText text1;
	protected HtmlOutputLabel label3;
	protected HtmlOutputText text2;
	protected HtmlOutputLabel label4;
	protected HtmlOutputText text3;
	protected HtmlOutputLabel label5;
	protected HtmlOutputText text4;
	protected HtmlOutputLabel label6;
	protected HtmlOutputText text5;
	protected HtmlOutputLabel label8;
	protected HtmlOutputText text7;
	protected HtmlOutputLabel label7;
	protected HtmlOutputText text6;
	protected HtmlOutputLabel label9;
	protected HtmlOutputText text8;
	protected HtmlOutputLabel label25;
	protected HtmlOutputText text26;
	protected HtmlPanelGroup group1;
	protected HtmlOutputLabel label1;
	protected HtmlPanelGrid grid4;
	protected HtmlCommandButton button1;
	protected PanelTab bfpanel2;
	protected HtmlPanelGrid grid2;
	protected HtmlOutputLabel label11;
	protected HtmlOutputText text9;
	protected HtmlOutputLabel label12;
	protected HtmlOutputText text10;
	protected HtmlOutputLabel label13;
	protected HtmlOutputText text11;
	protected HtmlOutputLabel label14;
	protected HtmlOutputText text12;
	protected HtmlOutputLabel label15;
	protected HtmlOutputText text13;
	protected HtmlOutputLabel label16;
	protected HtmlOutputText text14;
	protected HtmlOutputLabel label17;
	protected HtmlOutputText text15;
	protected HtmlOutputLabel label18;
	protected HtmlOutputText text16;
	protected HtmlPanelGroup group2;
	protected HtmlOutputLabel label10;
	protected PanelTab bfpanel3;
	protected HtmlDataTable tableEx1;
	protected HtmlOutputLabel label19;
	protected UIColumn column1;
	protected HtmlOutputText text17;
	protected HtmlOutputText text20;
	protected UIColumn column201;
	protected HtmlOutputText text456;
	protected HtmlOutputText text111;
	protected UIColumn column3;
	protected HtmlCommandLink link1;
	protected HtmlOutputText text19;
	protected UIColumn column35;
	protected HtmlCommandLink link14;
	protected HtmlOutputText text195;
	protected HtmlDataTable tableEx2;
	protected HtmlOutputText text284;
	protected HtmlOutputText text22;
	protected HtmlOutputText text221;
	protected HtmlOutputText text23;
	protected HtmlOutputText text222;
	protected HtmlOutputText text24;
	protected HtmlOutputText text223;
	protected HtmlOutputText text25;
	protected HtmlOutputText text27;
	protected HtmlOutputText text28;
	protected UIColumn columnEx1;
	protected UIColumn columnEx2;
	protected UIColumn columnEx3;
	protected UIColumn columnEx4;
	protected UIColumn columnEx5;
	protected UIColumn columnEx6;
	protected UIColumn columnEx668;
	protected UIColumn columnEx67;
	protected HtmlOutputText text994;
	protected HtmlCommandLink link69;
	protected HtmlCommandLink link68;
	protected HtmlOutputText text194;
	protected UIColumn columnEx46;
	protected HtmlOutputText text278;
	protected HtmlOutputText text331;
	protected HtmlCommandLink link99;
	protected UIColumn columnEx88;
	protected HtmlCommandLink link6;
	protected UIColumn columnEx66;
	protected HtmlOutputText text294;
	protected UIColumn columnEx168;
	protected HtmlCommandLink link169;
	protected HtmlOutputText text394;
	protected UIColumn columnEx233;
	protected HtmlCommandLink link244;
	protected HtmlOutputText text255;
	protected HtmlOutputText text18;
	protected UIColumn column2;
	protected HtmlOutputText text21;
	protected PanelTab bfpanel46;
	protected HtmlDataTable tableEx26;
	protected HtmlOutputText text22z;
	protected HtmlOutputText text27z;
	protected UIColumn columnEx1z;
	protected HtmlOutputText text221z;
	protected UIColumn columnEx5z;
	protected UIColumn columnEx6z;
	protected HtmlCommandButton tabbedPanel1_back;
	protected HtmlCommandButton tabbedPanel1_next;
	protected HtmlCommandButton tabbedPanel1_finish;
	protected HtmlCommandButton tabbedPanel1_cancel;
	protected HtmlOutputText text28z;
	protected UIColumn columnEx668z;
	protected HtmlCommandLink link69z;
	protected HtmlOutputText text994z;
	protected UIColumn columnEx66z;
	protected HtmlCommandLink link6z;
	protected UIColumn columnEx67z;
	protected HtmlCommandLink link68z;
	protected HtmlOutputText text194z;
	protected HtmlCommandLink link68zty;
	protected HtmlOutputText text194zut;
	protected UIColumn columnEx6ty68z;
	protected HtmlCommandLink link6ir9z;
	protected HtmlOutputText text99it4z;
	protected UIColumn columnEx88x;
	protected UIColumn columnEx6238;
	protected UIColumn columnEx6237;
	protected HtmlOutputText text13494;
	protected HtmlMessages messages67;
	protected HtmlCommandLink link4599;
	protected HtmlCommandLink l8899;
	protected HtmlOutputText text23195;
	protected HtmlOutputLabel label134;
	protected HtmlOutputText tetr34xt13;
	protected UIColumn columnEx8845x;
	protected UIColumn deRegisterColumn23;
	protected HtmlCommandLink l12ink14;
	protected HtmlPanelGrid grid5;
	protected HtmlOutputText text31;
	protected HtmlOutputText text32;
	protected HtmlOutputText textdf29;
	protected HtmlOutputText te8923;
	protected HtmlOutputText dee8923;
	protected HtmlCommandLink link4;
	protected HtmlCommandLink link5;
	protected HtmlCommandLink link223;
	protected HtmlCommandLink link903;
	protected HtmlCommandLink li903;
	protected HtmlCommandButton button2;
	protected HtmlPanelGrid grid341;
	protected HtmlOutputLabel label341;
	protected HtmlCommandLink link2;
	protected HtmlPanelGrid gridwe5;
	protected PanelTab getBfpanel4() {
		if (bfpanel4 == null) {
			bfpanel4 = (PanelTab) findComponentInRoot("bfpanel4");
		}
		return bfpanel4;
	}

	protected HtmlForm getForm1() {
		if (form1 == null) {
			form1 = (HtmlForm) findComponentInRoot("form1");
		}
		return form1;
	}

	protected HtmlScriptCollector getScriptCollector1() {
		if (scriptCollector1 == null) {
			scriptCollector1 = (HtmlScriptCollector) findComponentInRoot("scriptCollector1");
		}
		return scriptCollector1;
	}

	protected HtmlMessages getMessages1() {
		if (messages1 == null) {
			messages1 = (HtmlMessages) findComponentInRoot("messages1");
		}
		return messages1;
	}

	protected PanelTab getBfpanel1() {
		if (bfpanel1 == null) {
			bfpanel1 = (PanelTab) findComponentInRoot("bfpanel1");
		}
		return bfpanel1;
	}

	protected HtmlPanelGrid getGrid1() {
		if (grid1 == null) {
			grid1 = (HtmlPanelGrid) findComponentInRoot("grid1");
		}
		return grid1;
	}

	protected HtmlOutputLabel getLabel2() {
		if (label2 == null) {
			label2 = (HtmlOutputLabel) findComponentInRoot("label2");
		}
		return label2;
	}

	protected HtmlOutputText getText1() {
		if (text1 == null) {
			text1 = (HtmlOutputText) findComponentInRoot("text1");
		}
		return text1;
	}

	protected HtmlOutputLabel getLabel3() {
		if (label3 == null) {
			label3 = (HtmlOutputLabel) findComponentInRoot("label3");
		}
		return label3;
	}

	protected HtmlOutputText getText2() {
		if (text2 == null) {
			text2 = (HtmlOutputText) findComponentInRoot("text2");
		}
		return text2;
	}

	protected HtmlOutputLabel getLabel4() {
		if (label4 == null) {
			label4 = (HtmlOutputLabel) findComponentInRoot("label4");
		}
		return label4;
	}

	protected HtmlOutputText getText3() {
		if (text3 == null) {
			text3 = (HtmlOutputText) findComponentInRoot("text3");
		}
		return text3;
	}

	protected HtmlOutputLabel getLabel5() {
		if (label5 == null) {
			label5 = (HtmlOutputLabel) findComponentInRoot("label5");
		}
		return label5;
	}

	protected HtmlOutputText getText4() {
		if (text4 == null) {
			text4 = (HtmlOutputText) findComponentInRoot("text4");
		}
		return text4;
	}

	protected HtmlOutputLabel getLabel6() {
		if (label6 == null) {
			label6 = (HtmlOutputLabel) findComponentInRoot("label6");
		}
		return label6;
	}

	protected HtmlOutputText getText5() {
		if (text5 == null) {
			text5 = (HtmlOutputText) findComponentInRoot("text5");
		}
		return text5;
	}

	protected HtmlOutputLabel getLabel8() {
		if (label8 == null) {
			label8 = (HtmlOutputLabel) findComponentInRoot("label8");
		}
		return label8;
	}

	protected HtmlOutputText getText7() {
		if (text7 == null) {
			text7 = (HtmlOutputText) findComponentInRoot("text7");
		}
		return text7;
	}

	protected HtmlOutputLabel getLabel7() {
		if (label7 == null) {
			label7 = (HtmlOutputLabel) findComponentInRoot("label7");
		}
		return label7;
	}

	protected HtmlOutputText getText6() {
		if (text6 == null) {
			text6 = (HtmlOutputText) findComponentInRoot("text6");
		}
		return text6;
	}

	protected HtmlOutputLabel getLabel9() {
		if (label9 == null) {
			label9 = (HtmlOutputLabel) findComponentInRoot("label9");
		}
		return label9;
	}

	protected HtmlOutputText getText8() {
		if (text8 == null) {
			text8 = (HtmlOutputText) findComponentInRoot("text8");
		}
		return text8;
	}

	protected HtmlOutputLabel getLabel25() {
		if (label25 == null) {
			label25 = (HtmlOutputLabel) findComponentInRoot("label25");
		}
		return label25;
	}

	protected HtmlOutputText getText26() {
		if (text26 == null) {
			text26 = (HtmlOutputText) findComponentInRoot("text26");
		}
		return text26;
	}

	protected HtmlPanelGroup getGroup1() {
		if (group1 == null) {
			group1 = (HtmlPanelGroup) findComponentInRoot("group1");
		}
		return group1;
	}

	protected HtmlOutputLabel getLabel1() {
		if (label1 == null) {
			label1 = (HtmlOutputLabel) findComponentInRoot("label1");
		}
		return label1;
	}

	protected HtmlPanelGrid getGrid4() {
		if (grid4 == null) {
			grid4 = (HtmlPanelGrid) findComponentInRoot("grid4");
		}
		return grid4;
	}

	protected HtmlCommandButton getButton1() {
		if (button1 == null) {
			button1 = (HtmlCommandButton) findComponentInRoot("button1");
		}
		return button1;
	}

	protected PanelTab getBfpanel2() {
		if (bfpanel2 == null) {
			bfpanel2 = (PanelTab) findComponentInRoot("bfpanel2");
		}
		return bfpanel2;
	}

	protected HtmlPanelGrid getGrid2() {
		if (grid2 == null) {
			grid2 = (HtmlPanelGrid) findComponentInRoot("grid2");
		}
		return grid2;
	}

	protected HtmlOutputLabel getLabel11() {
		if (label11 == null) {
			label11 = (HtmlOutputLabel) findComponentInRoot("label11");
		}
		return label11;
	}

	protected HtmlOutputText getText9() {
		if (text9 == null) {
			text9 = (HtmlOutputText) findComponentInRoot("text9");
		}
		return text9;
	}

	protected HtmlOutputLabel getLabel12() {
		if (label12 == null) {
			label12 = (HtmlOutputLabel) findComponentInRoot("label12");
		}
		return label12;
	}

	protected HtmlOutputText getText10() {
		if (text10 == null) {
			text10 = (HtmlOutputText) findComponentInRoot("text10");
		}
		return text10;
	}

	protected HtmlOutputLabel getLabel13() {
		if (label13 == null) {
			label13 = (HtmlOutputLabel) findComponentInRoot("label13");
		}
		return label13;
	}

	protected HtmlOutputText getText11() {
		if (text11 == null) {
			text11 = (HtmlOutputText) findComponentInRoot("text11");
		}
		return text11;
	}

	protected HtmlOutputLabel getLabel14() {
		if (label14 == null) {
			label14 = (HtmlOutputLabel) findComponentInRoot("label14");
		}
		return label14;
	}

	protected HtmlOutputText getText12() {
		if (text12 == null) {
			text12 = (HtmlOutputText) findComponentInRoot("text12");
		}
		return text12;
	}

	protected HtmlOutputLabel getLabel15() {
		if (label15 == null) {
			label15 = (HtmlOutputLabel) findComponentInRoot("label15");
		}
		return label15;
	}

	protected HtmlOutputText getText13() {
		if (text13 == null) {
			text13 = (HtmlOutputText) findComponentInRoot("text13");
		}
		return text13;
	}

	protected HtmlOutputLabel getLabel16() {
		if (label16 == null) {
			label16 = (HtmlOutputLabel) findComponentInRoot("label16");
		}
		return label16;
	}

	protected HtmlOutputText getText14() {
		if (text14 == null) {
			text14 = (HtmlOutputText) findComponentInRoot("text14");
		}
		return text14;
	}

	protected HtmlOutputLabel getLabel17() {
		if (label17 == null) {
			label17 = (HtmlOutputLabel) findComponentInRoot("label17");
		}
		return label17;
	}

	protected HtmlOutputText getText15() {
		if (text15 == null) {
			text15 = (HtmlOutputText) findComponentInRoot("text15");
		}
		return text15;
	}

	protected HtmlOutputLabel getLabel18() {
		if (label18 == null) {
			label18 = (HtmlOutputLabel) findComponentInRoot("label18");
		}
		return label18;
	}

	protected HtmlOutputText getText16() {
		if (text16 == null) {
			text16 = (HtmlOutputText) findComponentInRoot("text16");
		}
		return text16;
	}

	protected HtmlPanelGroup getGroup2() {
		if (group2 == null) {
			group2 = (HtmlPanelGroup) findComponentInRoot("group2");
		}
		return group2;
	}

	protected HtmlOutputLabel getLabel10() {
		if (label10 == null) {
			label10 = (HtmlOutputLabel) findComponentInRoot("label10");
		}
		return label10;
	}

	protected PanelTab getBfpanel3() {
		if (bfpanel3 == null) {
			bfpanel3 = (PanelTab) findComponentInRoot("bfpanel3");
		}
		return bfpanel3;
	}

	protected HtmlDataTable getTableEx1() {
		if (tableEx1 == null) {
			tableEx1 = (HtmlDataTable) findComponentInRoot("tableEx1");
		}
		return tableEx1;
	}

	protected HtmlOutputLabel getLabel19() {
		if (label19 == null) {
			label19 = (HtmlOutputLabel) findComponentInRoot("label19");
		}
		return label19;
	}

	protected UIColumn getColumn1() {
		if (column1 == null) {
			column1 = (UIColumn) findComponentInRoot("column1");
		}
		return column1;
	}

	protected HtmlOutputText getText17() {
		if (text17 == null) {
			text17 = (HtmlOutputText) findComponentInRoot("text17");
		}
		return text17;
	}

	protected HtmlOutputText getText20() {
		if (text20 == null) {
			text20 = (HtmlOutputText) findComponentInRoot("text20");
		}
		return text20;
	}

	protected UIColumn getColumn201() {
		if (column201 == null) {
			column201 = (UIColumn) findComponentInRoot("column201");
		}
		return column201;
	}

	protected HtmlOutputText getText456() {
		if (text456 == null) {
			text456 = (HtmlOutputText) findComponentInRoot("text456");
		}
		return text456;
	}

	protected HtmlOutputText getText111() {
		if (text111 == null) {
			text111 = (HtmlOutputText) findComponentInRoot("text111");
		}
		return text111;
	}

	protected UIColumn getColumn3() {
		if (column3 == null) {
			column3 = (UIColumn) findComponentInRoot("column3");
		}
		return column3;
	}

	protected HtmlCommandLink getLink1() {
		if (link1 == null) {
			link1 = (HtmlCommandLink) findComponentInRoot("link1");
		}
		return link1;
	}

	protected HtmlOutputText getText19() {
		if (text19 == null) {
			text19 = (HtmlOutputText) findComponentInRoot("text19");
		}
		return text19;
	}

	protected UIColumn getColumn35() {
		if (column35 == null) {
			column35 = (UIColumn) findComponentInRoot("column35");
		}
		return column35;
	}

	protected HtmlCommandLink getLink14() {
		if (link14 == null) {
			link14 = (HtmlCommandLink) findComponentInRoot("link14");
		}
		return link14;
	}

	protected HtmlOutputText getText195() {
		if (text195 == null) {
			text195 = (HtmlOutputText) findComponentInRoot("text195");
		}
		return text195;
	}

	protected HtmlDataTable getTableEx2() {
		if (tableEx2 == null) {
			tableEx2 = (HtmlDataTable) findComponentInRoot("tableEx2");
		}
		return tableEx2;
	}

	protected HtmlOutputText getText284() {
		if (text284 == null) {
			text284 = (HtmlOutputText) findComponentInRoot("text284");
		}
		return text284;
	}

	protected HtmlOutputText getText22() {
		if (text22 == null) {
			text22 = (HtmlOutputText) findComponentInRoot("text22");
		}
		return text22;
	}

	protected HtmlOutputText getText221() {
		if (text221 == null) {
			text221 = (HtmlOutputText) findComponentInRoot("text221");
		}
		return text221;
	}

	protected HtmlOutputText getText23() {
		if (text23 == null) {
			text23 = (HtmlOutputText) findComponentInRoot("text23");
		}
		return text23;
	}

	protected HtmlOutputText getText222() {
		if (text222 == null) {
			text222 = (HtmlOutputText) findComponentInRoot("text222");
		}
		return text222;
	}

	protected HtmlOutputText getText24() {
		if (text24 == null) {
			text24 = (HtmlOutputText) findComponentInRoot("text24");
		}
		return text24;
	}

	protected HtmlOutputText getText223() {
		if (text223 == null) {
			text223 = (HtmlOutputText) findComponentInRoot("text223");
		}
		return text223;
	}

	protected HtmlOutputText getText25() {
		if (text25 == null) {
			text25 = (HtmlOutputText) findComponentInRoot("text25");
		}
		return text25;
	}

	protected HtmlOutputText getText27() {
		if (text27 == null) {
			text27 = (HtmlOutputText) findComponentInRoot("text27");
		}
		return text27;
	}

	protected HtmlOutputText getText28() {
		if (text28 == null) {
			text28 = (HtmlOutputText) findComponentInRoot("text28");
		}
		return text28;
	}

	protected UIColumn getColumnEx1() {
		if (columnEx1 == null) {
			columnEx1 = (UIColumn) findComponentInRoot("columnEx1");
		}
		return columnEx1;
	}

	protected UIColumn getColumnEx2() {
		if (columnEx2 == null) {
			columnEx2 = (UIColumn) findComponentInRoot("columnEx2");
		}
		return columnEx2;
	}

	protected UIColumn getColumnEx3() {
		if (columnEx3 == null) {
			columnEx3 = (UIColumn) findComponentInRoot("columnEx3");
		}
		return columnEx3;
	}

	protected UIColumn getColumnEx4() {
		if (columnEx4 == null) {
			columnEx4 = (UIColumn) findComponentInRoot("columnEx4");
		}
		return columnEx4;
	}

	protected UIColumn getColumnEx5() {
		if (columnEx5 == null) {
			columnEx5 = (UIColumn) findComponentInRoot("columnEx5");
		}
		return columnEx5;
	}

	protected UIColumn getColumnEx6() {
		if (columnEx6 == null) {
			columnEx6 = (UIColumn) findComponentInRoot("columnEx6");
		}
		return columnEx6;
	}

	protected UIColumn getColumnEx668() {
		if (columnEx668 == null) {
			columnEx668 = (UIColumn) findComponentInRoot("columnEx668");
		}
		return columnEx668;
	}

	protected UIColumn getColumnEx67() {
		if (columnEx67 == null) {
			columnEx67 = (UIColumn) findComponentInRoot("columnEx67");
		}
		return columnEx67;
	}

	protected HtmlOutputText getText994() {
		if (text994 == null) {
			text994 = (HtmlOutputText) findComponentInRoot("text994");
		}
		return text994;
	}

	protected HtmlCommandLink getLink69() {
		if (link69 == null) {
			link69 = (HtmlCommandLink) findComponentInRoot("link69");
		}
		return link69;
	}

	protected HtmlCommandLink getLink68() {
		if (link68 == null) {
			link68 = (HtmlCommandLink) findComponentInRoot("link68");
		}
		return link68;
	}

	protected HtmlOutputText getText194() {
		if (text194 == null) {
			text194 = (HtmlOutputText) findComponentInRoot("text194");
		}
		return text194;
	}

	protected UIColumn getColumnEx46() {
		if (columnEx46 == null) {
			columnEx46 = (UIColumn) findComponentInRoot("columnEx46");
		}
		return columnEx46;
	}

	protected HtmlOutputText getText278() {
		if (text278 == null) {
			text278 = (HtmlOutputText) findComponentInRoot("text278");
		}
		return text278;
	}

	protected HtmlOutputText getText331() {
		if (text331 == null) {
			text331 = (HtmlOutputText) findComponentInRoot("text331");
		}
		return text331;
	}

	protected HtmlCommandLink getLink99() {
		if (link99 == null) {
			link99 = (HtmlCommandLink) findComponentInRoot("link99");
		}
		return link99;
	}

	protected UIColumn getColumnEx88() {
		if (columnEx88 == null) {
			columnEx88 = (UIColumn) findComponentInRoot("columnEx88");
		}
		return columnEx88;
	}

	protected HtmlCommandLink getLink6() {
		if (link6 == null) {
			link6 = (HtmlCommandLink) findComponentInRoot("link6");
		}
		return link6;
	}

	protected UIColumn getColumnEx66() {
		if (columnEx66 == null) {
			columnEx66 = (UIColumn) findComponentInRoot("columnEx66");
		}
		return columnEx66;
	}

	protected HtmlOutputText getText294() {
		if (text294 == null) {
			text294 = (HtmlOutputText) findComponentInRoot("text294");
		}
		return text294;
	}

	protected UIColumn getColumnEx168() {
		if (columnEx168 == null) {
			columnEx168 = (UIColumn) findComponentInRoot("columnEx168");
		}
		return columnEx168;
	}

	protected HtmlCommandLink getLink169() {
		if (link169 == null) {
			link169 = (HtmlCommandLink) findComponentInRoot("link169");
		}
		return link169;
	}

	protected HtmlOutputText getText394() {
		if (text394 == null) {
			text394 = (HtmlOutputText) findComponentInRoot("text394");
		}
		return text394;
	}

	protected UIColumn getColumnEx233() {
		if (columnEx233 == null) {
			columnEx233 = (UIColumn) findComponentInRoot("columnEx233");
		}
		return columnEx233;
	}

	protected HtmlCommandLink getLink244() {
		if (link244 == null) {
			link244 = (HtmlCommandLink) findComponentInRoot("link244");
		}
		return link244;
	}

	protected HtmlOutputText getText255() {
		if (text255 == null) {
			text255 = (HtmlOutputText) findComponentInRoot("text255");
		}
		return text255;
	}

	protected HtmlOutputText getText18() {
		if (text18 == null) {
			text18 = (HtmlOutputText) findComponentInRoot("text18");
		}
		return text18;
	}

	protected UIColumn getColumn2() {
		if (column2 == null) {
			column2 = (UIColumn) findComponentInRoot("column2");
		}
		return column2;
	}

	protected HtmlOutputText getText21() {
		if (text21 == null) {
			text21 = (HtmlOutputText) findComponentInRoot("text21");
		}
		return text21;
	}

	protected PanelTab getBfpanel46() {
		if (bfpanel46 == null) {
			bfpanel46 = (PanelTab) findComponentInRoot("bfpanel46");
		}
		return bfpanel46;
	}

	protected HtmlDataTable getTableEx26() {
		if (tableEx26 == null) {
			tableEx26 = (HtmlDataTable) findComponentInRoot("tableEx26");
		}
		return tableEx26;
	}

	protected HtmlOutputText getText22z() {
		if (text22z == null) {
			text22z = (HtmlOutputText) findComponentInRoot("text22z");
		}
		return text22z;
	}

	protected HtmlOutputText getText27z() {
		if (text27z == null) {
			text27z = (HtmlOutputText) findComponentInRoot("text27z");
		}
		return text27z;
	}

	protected UIColumn getColumnEx1z() {
		if (columnEx1z == null) {
			columnEx1z = (UIColumn) findComponentInRoot("columnEx1z");
		}
		return columnEx1z;
	}

	protected HtmlOutputText getText221z() {
		if (text221z == null) {
			text221z = (HtmlOutputText) findComponentInRoot("text221z");
		}
		return text221z;
	}

	protected UIColumn getColumnEx5z() {
		if (columnEx5z == null) {
			columnEx5z = (UIColumn) findComponentInRoot("columnEx5z");
		}
		return columnEx5z;
	}

	protected UIColumn getColumnEx6z() {
		if (columnEx6z == null) {
			columnEx6z = (UIColumn) findComponentInRoot("columnEx6z");
		}
		return columnEx6z;
	}

	protected HtmlCommandButton getTabbedPanel1_back() {
		if (tabbedPanel1_back == null) {
			tabbedPanel1_back = (HtmlCommandButton) findComponentInRoot("tabbedPanel1_back");
		}
		return tabbedPanel1_back;
	}

	protected HtmlCommandButton getTabbedPanel1_next() {
		if (tabbedPanel1_next == null) {
			tabbedPanel1_next = (HtmlCommandButton) findComponentInRoot("tabbedPanel1_next");
		}
		return tabbedPanel1_next;
	}

	protected HtmlCommandButton getTabbedPanel1_finish() {
		if (tabbedPanel1_finish == null) {
			tabbedPanel1_finish = (HtmlCommandButton) findComponentInRoot("tabbedPanel1_finish");
		}
		return tabbedPanel1_finish;
	}

	protected HtmlCommandButton getTabbedPanel1_cancel() {
		if (tabbedPanel1_cancel == null) {
			tabbedPanel1_cancel = (HtmlCommandButton) findComponentInRoot("tabbedPanel1_cancel");
		}
		return tabbedPanel1_cancel;
	}

	protected HtmlOutputText getText28z() {
		if (text28z == null) {
			text28z = (HtmlOutputText) findComponentInRoot("text28z");
		}
		return text28z;
	}

	protected UIColumn getColumnEx668z() {
		if (columnEx668z == null) {
			columnEx668z = (UIColumn) findComponentInRoot("columnEx668z");
		}
		return columnEx668z;
	}

	protected HtmlCommandLink getLink69z() {
		if (link69z == null) {
			link69z = (HtmlCommandLink) findComponentInRoot("link69z");
		}
		return link69z;
	}

	protected HtmlOutputText getText994z() {
		if (text994z == null) {
			text994z = (HtmlOutputText) findComponentInRoot("text994z");
		}
		return text994z;
	}

	protected UIColumn getColumnEx66z() {
		if (columnEx66z == null) {
			columnEx66z = (UIColumn) findComponentInRoot("columnEx66z");
		}
		return columnEx66z;
	}

	protected HtmlCommandLink getLink6z() {
		if (link6z == null) {
			link6z = (HtmlCommandLink) findComponentInRoot("link6z");
		}
		return link6z;
	}

	protected UIColumn getColumnEx67z() {
		if (columnEx67z == null) {
			columnEx67z = (UIColumn) findComponentInRoot("columnEx67z");
		}
		return columnEx67z;
	}

	protected HtmlCommandLink getLink68z() {
		if (link68z == null) {
			link68z = (HtmlCommandLink) findComponentInRoot("link68z");
		}
		return link68z;
	}

	protected HtmlOutputText getText194z() {
		if (text194z == null) {
			text194z = (HtmlOutputText) findComponentInRoot("text194z");
		}
		return text194z;
	}

	protected HtmlCommandLink getLink68zty() {
		if (link68zty == null) {
			link68zty = (HtmlCommandLink) findComponentInRoot("link68zty");
		}
		return link68zty;
	}

	protected HtmlOutputText getText194zut() {
		if (text194zut == null) {
			text194zut = (HtmlOutputText) findComponentInRoot("text194zut");
		}
		return text194zut;
	}

	protected UIColumn getColumnEx6ty68z() {
		if (columnEx6ty68z == null) {
			columnEx6ty68z = (UIColumn) findComponentInRoot("columnEx6ty68z");
		}
		return columnEx6ty68z;
	}

	protected HtmlCommandLink getLink6ir9z() {
		if (link6ir9z == null) {
			link6ir9z = (HtmlCommandLink) findComponentInRoot("link6ir9z");
		}
		return link6ir9z;
	}

	protected HtmlOutputText getText99it4z() {
		if (text99it4z == null) {
			text99it4z = (HtmlOutputText) findComponentInRoot("text99it4z");
		}
		return text99it4z;
	}

	protected UIColumn getColumnEx88x() {
		if (columnEx88x == null) {
			columnEx88x = (UIColumn) findComponentInRoot("columnEx88x");
		}
		return columnEx88x;
	}

	protected UIColumn getColumnEx6238() {
		if (columnEx6238 == null) {
			columnEx6238 = (UIColumn) findComponentInRoot("columnEx6238");
		}
		return columnEx6238;
	}

	protected UIColumn getColumnEx6237() {
		if (columnEx6237 == null) {
			columnEx6237 = (UIColumn) findComponentInRoot("columnEx6237");
		}
		return columnEx6237;
	}

	protected HtmlOutputText getText13494() {
		if (text13494 == null) {
			text13494 = (HtmlOutputText) findComponentInRoot("text13494");
		}
		return text13494;
	}

	protected HtmlMessages getMessages67() {
		if (messages67 == null) {
			messages67 = (HtmlMessages) findComponentInRoot("messages67");
		}
		return messages67;
	}

	protected HtmlCommandLink getLink4599() {
		if (link4599 == null) {
			link4599 = (HtmlCommandLink) findComponentInRoot("link4599");
		}
		return link4599;
	}

	protected HtmlCommandLink getL8899() {
		if (l8899 == null) {
			l8899 = (HtmlCommandLink) findComponentInRoot("l8899");
		}
		return l8899;
	}

	protected HtmlOutputText getText23195() {
		if (text23195 == null) {
			text23195 = (HtmlOutputText) findComponentInRoot("text23195");
		}
		return text23195;
	}

	protected HtmlOutputLabel getLabel134() {
		if (label134 == null) {
			label134 = (HtmlOutputLabel) findComponentInRoot("label134");
		}
		return label134;
	}

	protected HtmlOutputText getTetr34xt13() {
		if (tetr34xt13 == null) {
			tetr34xt13 = (HtmlOutputText) findComponentInRoot("tetr34xt13");
		}
		return tetr34xt13;
	}

	protected UIColumn getColumnEx8845x() {
		if (columnEx8845x == null) {
			columnEx8845x = (UIColumn) findComponentInRoot("columnEx8845x");
		}
		return columnEx8845x;
	}

	protected UIColumn getDeRegisterColumn23() {
		if (deRegisterColumn23 == null) {
			deRegisterColumn23 = (UIColumn) findComponentInRoot("deRegisterColumn23");
		}
		return deRegisterColumn23;
	}

	protected HtmlCommandLink getL12ink14() {
		if (l12ink14 == null) {
			l12ink14 = (HtmlCommandLink) findComponentInRoot("l12ink14");
		}
		return l12ink14;
	}

	protected HtmlPanelGrid getGrid5() {
		if (grid5 == null) {
			grid5 = (HtmlPanelGrid) findComponentInRoot("grid5");
		}
		return grid5;
	}

	protected HtmlOutputText getText31() {
		if (text31 == null) {
			text31 = (HtmlOutputText) findComponentInRoot("text31");
		}
		return text31;
	}

	protected HtmlOutputText getText32() {
		if (text32 == null) {
			text32 = (HtmlOutputText) findComponentInRoot("text32");
		}
		return text32;
	}

	protected HtmlOutputText getTextdf29() {
		if (textdf29 == null) {
			textdf29 = (HtmlOutputText) findComponentInRoot("textdf29");
		}
		return textdf29;
	}

	protected HtmlOutputText getTe8923() {
		if (te8923 == null) {
			te8923 = (HtmlOutputText) findComponentInRoot("te8923");
		}
		return te8923;
	}

	protected HtmlOutputText getDee8923() {
		if (dee8923 == null) {
			dee8923 = (HtmlOutputText) findComponentInRoot("dee8923");
		}
		return dee8923;
	}

	protected HtmlCommandLink getLink4() {
		if (link4 == null) {
			link4 = (HtmlCommandLink) findComponentInRoot("link4");
		}
		return link4;
	}

	protected HtmlCommandLink getLink5() {
		if (link5 == null) {
			link5 = (HtmlCommandLink) findComponentInRoot("link5");
		}
		return link5;
	}

	protected HtmlCommandLink getLink223() {
		if (link223 == null) {
			link223 = (HtmlCommandLink) findComponentInRoot("link223");
		}
		return link223;
	}

	protected HtmlCommandLink getLink903() {
		if (link903 == null) {
			link903 = (HtmlCommandLink) findComponentInRoot("link903");
		}
		return link903;
	}

	protected HtmlCommandLink getLi903() {
		if (li903 == null) {
			li903 = (HtmlCommandLink) findComponentInRoot("li903");
		}
		return li903;
	}

	protected HtmlCommandButton getButton2() {
		if (button2 == null) {
			button2 = (HtmlCommandButton) findComponentInRoot("button2");
		}
		return button2;
	}

	protected HtmlPanelGrid getGrid341() {
		if (grid341 == null) {
			grid341 = (HtmlPanelGrid) findComponentInRoot("grid341");
		}
		return grid341;
	}

	protected HtmlOutputLabel getLabel341() {
		if (label341 == null) {
			label341 = (HtmlOutputLabel) findComponentInRoot("label341");
		}
		return label341;
	}

	protected HtmlCommandLink getLink2() {
		if (link2 == null) {
			link2 = (HtmlCommandLink) findComponentInRoot("link2");
		}
		return link2;
	}

	protected HtmlPanelGrid getGridwe5() {
		if (gridwe5 == null) {
			gridwe5 = (HtmlPanelGrid) findComponentInRoot("gridwe5");
		}
		return gridwe5;
	}

}