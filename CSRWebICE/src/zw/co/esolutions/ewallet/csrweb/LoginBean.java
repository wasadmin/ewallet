...
public class LoginBean extends PageCodeBase {
	...
	private void auditLogin(){
		try {
			...
			super.getAuditService().logActivityWithNarrative(super.getJaasUserName(), AuditEvents.LOGIN, "CSR APPLICATION USER LOGIN FROM IP ADDRESS:"+ip);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void auditLogout(){
		try {
			...
			super.getAuditService().logActivityWithNarrative(super.getJaasUserName(), AuditEvents.LOGOUT, "CSR APPLICATION USER LOGOUT FROM IP ADDRESS:"+ip);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
