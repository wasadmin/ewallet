 ...
 *
 */
public class SettlementReportBean extends PageCodeBase{
	...
	public String generateReport() {
		...
		this.setTxnStatusItem(TransactionStatus.COMPLETED.toString());
		...
		if("all".equalsIgnoreCase(this.getTxnGrpItem()) && "all".equalsIgnoreCase(this.getTxnItem())) {
			if(queryExtension == null) {
				queryExtension = "AND "+super.getSettlementTxnQuery();
			} else {
				queryExtension = queryExtension + "AND "+super.getSettlementTxnQuery();
			}
		} else {
				...
				if(!this.getTxnItem().equalsIgnoreCase("all")) {
					...
					if(queryExtension == null) {
						if(TransactionType.TOPUP.equals(txnType) || TransactionType.BILLPAY.equals(txnType)) {
							...
							queryExtension = "AND transactionType = '"+this.getTxnItem()+"' ";
						}
					} else {
						if(TransactionType.TOPUP.equals(txnType) || TransactionType.BILLPAY.equals(txnType)) {
							queryExtension = queryExtension+"AND "+super.getTxnGrpQuery(this.getTxnGrpItem());
						} else {
							queryExtension = queryExtension+"AND transactionType = '"+this.getTxnItem()+"' ";
						}
			...
			if(this.getBankItem().equalsIgnoreCase("all")) {
				for(Bank bnk : bankService.getBank()) {
				    if(bnk != null) {
				    	...
				    	parameters = super.populateSMS(parameters, bnk.getId());
				...
				parameters.put("reportTitle", "Settlement Report for all Banks");
		        parameters.put("pageHeader", "All Banks : Branch Settlement Report from "+DateUtil.convertToDateWithTime(this.getFromDate())+
				" to "+DateUtil.convertToDateWithTime(this.getToDate()));
		        
				query = "SELECT p.dateCreated, valueDate, messageId, transactionType, sourceMobile, sourceAccountNumber, targetMobile, destinationAccountNumber, amount, utilityAccount, p.status, responseCode, narrative, branchId, fromBankId, transactionLocationType, transactionLocationId, balance, customerName  FROM "+EWalletConstants.DATABASE_SCHEMA+".PROCESSTRANSACTION AS p, " +
						""+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH, " +			
						""+EWalletConstants.DATABASE_SCHEMA+".BANK WHERE " +
						" ("+EWalletConstants.DATABASE_SCHEMA+".BANK.id = fromBankId) " +
						"AND "+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH.ID = branchId "; 
				orderBy ="ORDER BY "+EWalletConstants.DATABASE_SCHEMA+".BANK.NAME ASC, "+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH.NAME ASC, p.dateCreated DESC ";
				...
				if(this.getBranchItem().equalsIgnoreCase("all")) {
					...
					if(!(this.getBankBranchList() == null || this.getBankBranchList().isEmpty())) {
						...
						parameters.put("pageHeader", "All "+parameters.get(this.getBankItem()).toString()+
								" Branches Settlement Report from "+DateUtil.convertToDateWithTime(this.getFromDate())+
								" to "+DateUtil.convertToDateWithTime(this.getToDate()));
					}
					
					query = "SELECT p.dateCreated, valueDate, messageId, transactionType, sourceMobile, sourceAccountNumber, targetMobile, destinationAccountNumber, amount, utilityAccount, p.status, responseCode, narrative, branchId, fromBankId, transactionLocationType, transactionLocationId, balance, customerName FROM " +
							""+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH, " +
							""+EWalletConstants.DATABASE_SCHEMA+".BANK, " +
							EWalletConstants.DATABASE_SCHEMA+".PROCESSTRANSACTION AS p WHERE " +
							" ("+EWalletConstants.DATABASE_SCHEMA+".BANK.id = fromBankId) " +
							"AND fromBankId = '"+this.getBankItem()+"' "+
							"AND "+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH.ID = branchId ";
					
					orderBy ="ORDER BY "+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH.NAME ASC, p.dateCreated DESC ";
					...
					" : "+bankService.findBankBranchById(this.getBranchItem()).getName()+" Branch Settlement Report from "+DateUtil.convertToDateWithTime(this.getFromDate())+
					" to "+DateUtil.convertToDateWithTime(this.getToDate()));
			        
				
					query = "SELECT p.dateCreated, valueDate, messageId, transactionType, sourceMobile, sourceAccountNumber, targetMobile, destinationAccountNumber, amount, utilityAccount, p.status, responseCode, narrative, branchId, fromBankId, transactionLocationType, transactionLocationId, balance, customerName FROM "+EWalletConstants.DATABASE_SCHEMA+".PROCESSTRANSACTION AS p WHERE " +
		...
		try {
			ctx.redirect("/EWalletReportsWebICE/jspviewer/viewer.jsp?pdfName="+URLEncryptor.encryptUrl(fileId+fileName+".pdf")+"&query="+URLEncryptor.encryptUrl(query)+"&"+EWalletConstants.MASTER_REPORT+"="+URLEncryptor.encryptUrl(MapUtil.convertAttributesMapToString(parameters))+"&pageName="+URLEncryptor.encryptUrl("settlementReport.jspx"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		return "success";
		...
		if(item != null) {
			try {
				...
				if(!(item.equalsIgnoreCase("all") || item.equalsIgnoreCase("none"))) {
					...
					for(TransactionType txn : super.getSettlementTxnTypes()) {
						txnMenu.add(new SelectItem(txn.toString(), txn.toString()));
					}
				}
	...
	public List<SelectItem> getTxnMenu() {
		if(this.txnMenu == null) {
			...
			//this.txnMenu.add(new SelectItem("none", "--select--"));
			this.txnMenu.add(new SelectItem("all", "ALL"));
//			for(TransactionType txnType : super.getSettlementTxnTypes()) {
//				if(TransactionType.BALANCE.equals(txnType)) {
//					//Do Nothing Here
//				} else {
//					if(TransactionType.BALANCE_REQUEST.equals(txnType)) {
//						this.txnMenu.add(new SelectItem(TransactionType.BALANCE.toString(), txnType.toString()));
//					} else {
