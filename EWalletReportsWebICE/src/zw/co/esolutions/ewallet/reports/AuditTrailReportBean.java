...
public class AuditTrailReportBean extends PageCodeBase{
	...
	private List<SelectItem> activityMenu;
	...
	public String generateReport() {
		...
		String queryExt = " at.username = p.username AND p.role_id = r.id AND p.branchId = bc.id AND bc.bank_id = b.id AND a.id = at.activity_id ";
...
//			String auditTrailQuery = "";
			query = "SELECT b.name AS bankName, p.id AS profileId, p.lastname, p.branchId, p.firstnames,p.email,p.mobileNumber , r.roleName, a.name, b.id AS bankId,bc.name AS branchName, at.userName,at.narrative ,at.time FROM "+
			"(SELECT * FROM "+EWalletConstants.DATABASE_SCHEMA+".AUDITTRAIL at where at.ACTIVITY_ID LIKE '%"+getActivityItem()+"%' "+
			...
			+EWalletConstants.DATABASE_SCHEMA+".BANK as b, "+EWalletConstants.DATABASE_SCHEMA+".ROLE as r, "+EWalletConstants.DATABASE_SCHEMA+".ACTIVITY as a, "+EWalletConstants.DATABASE_SCHEMA+".BANKBRANCH as bc WHERE ";
			...
			if(this.getBankItem().equalsIgnoreCase("all")) {
				...
				parameters.put("reportTitle", "eWallet Audit Trail for all Banks ");
				
				parameters.put("pageHeader", "All Banks : Audit Trail from "+ DateUtil.convertToDateWithTime(this.getFromDate())
				...
				if(this.getBranchItem().equalsIgnoreCase("all")) {
					...
					if(!(this.getBankBranchList() == null || this.getBankBranchList().isEmpty())) {
						...
						parameters.put("reportTitle", parameters.get(this.getBankItem()).toString()+" " +
								"Ewallet Audit Trail Report");
						parameters.put("pageHeader", "All "+ parameters.get(this.getBankItem()).toString()
								+ " EWallet Audit Trail from :"+ DateUtil.convertToDateWithTime(this.getFromDate())
								+ " to "+ DateUtil.convertToDateWithTime(this.getToDate()));
					}
					...
					parameters.put("pageHeader", parameters.get(this.getBankItem()).toString()+ " : "+ bankService.findBankBranchById(
							this.getBranchItem()).getName()+ " Branch Audit Trail From :"+ DateUtil.convertToDateWithTime(this.getFromDate())
					+ " to "+ DateUtil.convertToDateWithTime(this.getToDate()));
		...
		try {
			ctx.redirect("/EWalletReportsWebICE/jspviewer/viewer.jsp?pdfName="+URLEncryptor.encryptUrl(fileId+fileName+".pdf")+"&query="+URLEncryptor.encryptUrl(query)+"&"+EWalletConstants.MASTER_REPORT+"="+URLEncryptor.encryptUrl(MapUtil.convertAttributesMapToString(parameters))+"&pageName="+URLEncryptor.encryptUrl("auditTrailReport.jspx"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		return "success";
...
//				roleMenu.add(new SelectItem("none", "<--select-->"));
	...
	public List<SelectItem> getActivityMenu() {
		
		if(this.activityMenu == null) {
			activityMenu = new ArrayList<SelectItem>();
			if(this.getBankList() == null || this.getActivityList().isEmpty()) {
				activityMenu.add(new SelectItem("nothing", "No Activities"));
			} else {
				activityMenu.add(new SelectItem("all", "All"));
				for(Activity activity : this.getActivityList()) {
					activityMenu.add(new SelectItem(activity.getId(),activity.getName()));
				}
			}
		}
		return activityMenu;
	...
	public void setActivityMenu(List<SelectItem> activityMenu) {
	...
	public List<Activity> getActivityList() {
		...
		if(this.activityList == null || this.activityList.isEmpty()) {
			try {
				activityList = auditService.getAllActivities();
			} catch (Exception e) {
