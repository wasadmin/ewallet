 ...
 *
 */
public class SearchBankTransactionsBean extends PageCodeBase{
	 ...
	 * 
	 */
	public SearchBankTransactionsBean() {
...
//		}
	...
	private String checkAttributes(){
		...
		if(this.getBankItem().equalsIgnoreCase("none")) {
			buffer.append("Bank, ");
		}if("none".equalsIgnoreCase(this.getTxnTypeItem()) || this.getTxnTypeItem() == null) {
	...
	public String viewPostings() {
		try {
			...
			super.getRequestScope().put("fromPage", "searchBankTransactions.jspx");
		} catch (Exception e) {
...
//				}
				...
				if(this.getBranch() != null) {
					this.bankMenu.add(new SelectItem(this.getBranchItem(), this.getBranch().getBank().getName()));
				}
			}
		}
		return bankMenu;
	...
	public List<SelectItem> getBranchMenu() {
		if(this.branchMenu == null) {
			if((this.branchMenu == null || this.branchMenu.isEmpty()) && this.getBankItem() != null) {
				try {
					BankServiceSOAPProxy bankService = new BankServiceSOAPProxy();
					List<BankBranch> branchList = bankService.getBankBranchByBank(this.getBankItem());
					this.branchMenu = new ArrayList<SelectItem>();
					if(branchList == null || branchList.isEmpty()) {
						this.branchMenu.add(new SelectItem("nothing", "No Branches"));
					} else {
						this.branchMenu.add(new SelectItem("all", "ALL"));
						for(BankBranch br : branchList) {
							this.branchMenu.add(new SelectItem(br.getId(), br.getName()));
						}
					}
				} catch (Exception e) {
					// TODO: handle exception
	...
	public List<BankBranch> getBankBranchList() {
		if(bankBranchList == null) {
			try {
				this.bankBranchList = super.getBankService().getBankBranchByBank(this.getBankItem());
			} catch (Exception e) {
				// TODO: handle exception
			}
		}
	...
	public BankBranch getBranch() {
		if(this.branch == null) {
			try {
				...
				branch = super.getBankService().findBankBranchById(p.getBranchId());
			} catch (Exception e) {
				// TODO: handle exception
			}
		}
