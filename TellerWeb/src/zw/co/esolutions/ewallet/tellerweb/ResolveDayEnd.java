...
public class ResolveDayEnd extends PageCodeBase {
...
private static Logger LOG ;
	...
	public boolean isApprover() {
		...
		this.approver=proxy.canDo(getJaasUserName(), "dayEndApproval");
		return approver;
	}
	...
	private void setNetBalance(List<DayEndSummary> summaries, DayEnd tellerDayEnd) {
		...
		netBalance=tellerFloat +depositSum-withDrawalSum-nonHolderSum;
	...
	public String resolveDayEnd(){
		ProcessServiceSOAPProxy proxy = new ProcessServiceSOAPProxy();
		dayEnd.setStatus(DayEndStatus.COMPLETED);
		try {
			DayEnd resultDayEnd=proxy.editDayEnd(dayEnd, getJaasUserName());
			if(resultDayEnd!=null && DayEndStatus.COMPLETED.equals(resultDayEnd.getStatus())){
				super.setInformationMessage("Day end has been marked as resolved. ");
			}else{
				super.setInformationMessage("Failed to update day end.");
				
			}
		} catch (Exception e) {
			super.setInformationMessage("Failed to update day end. Error message: "+e.getMessage());
		}
	...
	public String approvalProcess(){
		...
		try {
			...
			if(ResponseCode.E000.name().equalsIgnoreCase(response.getStatus())){
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			super.setInformationMessage("Error occured. Error Message "+e.getMessage());
		}
		this.approver=false;
	...
	public String disapprove(){
		...
		try {
			...
			super.setInformationMessage("Teller day end successfully disapproved.");
	...
	LOG.debug("   the value "+(search!=null));
		if(search!=null){
			gotoPage("/teller/viewdayEndsManualResolve.jspxx");
			return "";
		}
		gotoPage("/teller/viewdayEndsManualResolve.jspx");
		return "";
	}
	...
	public String viewPostings() {
		try {
			...
			super.getRequestScope().put("fromPage", "dayEndManualSummary.jspx");
