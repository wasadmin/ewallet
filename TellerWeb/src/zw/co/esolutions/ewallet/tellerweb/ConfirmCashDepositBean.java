 ...
 *
 */
public class ConfirmCashDepositBean extends PageCodeBase{
	...
	static{
		try {
			...
			LOG = Logger.getLogger(ConfirmCashDepositBean.class);
		} catch (Exception e) {
			System.err.println("Failed to initilise logger for " + ConfirmCashDepositBean.class);
		}
	}
	
	public ConfirmCashDepositBean() {
		super();
	...
	public String finish() {
		LOG.debug(">>>>>>>>>>>> Running the finish method In confirm cash deposit >>>>>>>>>");
		...
		try {
			...
			LOG.debug(">>>>>>>>>>>> In try block of ConfirmCashDeposit  >>>>>>>>>");
			...
			response = processService.validateDeposit(mobileProfile.getId(), info.getAmount(), this.getBranch().getBank().getId(), TransactionLocationType.BANK_BRANCH);
			
			if(!ResponseCode.E000.name().equalsIgnoreCase(response)) {
				super.setErrorMessage(response);
				return "failure";
			}
			processResponse = processService.depositCash(info);
			response = processResponse.getResponseCode();
			LOG.debug(">>>>>>>>>>>> Done depositing cash  >>>>>>>>>");
		} catch (Exception e) {
	...
	public String back() {
		super.gotoPage("/teller/depositCash.jspx");
		return "success";
	}
		...
		if(this.branch == null && this.getCustomer() != null) {
			try {
				...
				if(ewalletBankAccount==null || ewalletBankAccount.getId()==null){
					ewalletBankAccount=super.getBankService().getBankAccountByAccountHolderAndTypeAndOwnerType(this.getCustomer().getId(), BankAccountType.E_WALLET, OwnerType.CUSTOMER, this.getMobileNumber());
				}
	...
	public Limit getDeDailyLimit() {
		if(this.deDailyLimit == null && this.getBankAccount() != null) {
			try {
				this.deDailyLimit = super.getLimitService().getValidLimitOnDateByBankId(zw.co.esolutions.ewallet.limitservices.service.TransactionType.DEPOSIT,
						BankAccountClass.valueOf(this.getBankAccount().getAccountClass().toString()),
						DateUtil.convertToXMLGregorianCalendar(new Date()), LimitPeriodType.DAILY, 
						this.getBankAccount().getBranch().getBank().getId());
			} catch (Exception e) {
	...
	public Limit getDeTxnLimit() {
		if(this.deTxnLimit == null && this.getBankAccount() != null) {
			try {
				this.deTxnLimit = super.getLimitService().getValidLimitOnDateByBankId(zw.co.esolutions.ewallet.limitservices.service.TransactionType.DEPOSIT,
						BankAccountClass.valueOf(this.getBankAccount().getAccountClass().toString()),
						DateUtil.convertToXMLGregorianCalendar(new Date()), LimitPeriodType.TRANSACTION, 
						this.getBankAccount().getBranch().getBank().getId());
			} catch (Exception e) {
